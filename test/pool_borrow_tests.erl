...
%  ?debug_Fmt("    : pool borrow test: ~p",[X]),
  ...
  ?assertEqual({0,1}, f2()),
  Rsrc1 = resource_pool:borrow(Pool),
  ?assertEqual(0, tst_resource:get_id(Rsrc1)),
  Rsrc2 = resource_pool:borrow(Pool),
  tst_resource:set_id(Rsrc2, 1),
  Rsrc3 = resource_pool:borrow(Pool),
  tst_resource:set_id(Rsrc3, 2),
  Rsrc4 = resource_pool:borrow(Pool),
  tst_resource:set_id(Rsrc4, 3),
  ?assertEqual(4, f()),
  ?assertEqual({4,0}, f2()),
